...
public:
   StanleyControllerNode() : Node("stanley_controller")
   {
       ...
       this->declare_parameter<double>("Proportional", 0.4);
       this->declare_parameter<double>("Integral", 0.01);
       this->declare_parameter<double>("Derivative", 0.12);
       ...
       odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>("odom/sim", 10, std::bind(&StanleyControllerNode::odomCallback, this,_1));
       ...
       robot_path_pub_ = this->create_publisher<nav_msgs::msg::Path>("/robot_actual_path", 10);
   }
private:
   double K_P_;
   double K_I_;
   ...
   void set_velocity(int index)
    {
        const double base_velocity = 0.43;
   ...
   std::pair<double, double> computeError(double x, double y, double theta)
   {
       ...
       int target_index = std::min(closest_index + 2,(int)path_points_x_.size() - 1);
       ...
       double fx = x + L_ * cos(theta);
       double fy = y + L_ * sin(theta);
   ...
   {
       float current_time_ = this->now().seconds();
       double dt = 0.1;
       ...
       double control_command = heading_error + atan2((K_P_ * cross_track_error + K_I_ * integral + K_D_ * derivative), velocity_+1e-5);

       std_msgs::msg::Float64MultiArray control_msg;
       control_msg.data = {velocity_, control_command};
       control_pub_->publish(control_msg);

       double yaw_rate = (velocity_ / L_) * tan(control_command);
   ...
   void odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg)
   {
       ...
       if (abs(x - path_points_x_.back()) < 0.2 && abs(y - path_points_y_.back()) < 0.2) {
           velocity_ = 0.0;
           std_msgs::msg::Float64MultiArray control_msg;
           control_msg.data = {velocity_, 0.0};
           control_pub_->publish(control_msg);
           return;
       }
